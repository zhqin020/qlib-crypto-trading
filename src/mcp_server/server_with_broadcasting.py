"""
MCP Server with UI broadcasting support
"""
...
# Try to import event broadcaster (optional, won't break if UI not running)
try:
    ...
    broadcaster = None
    logger.warning("Event broadcaster not available - UI updates disabled")


async def broadcast_event(tool_name: str, arguments: Dict, result: Dict, status: str = "success"):
    """Broadcast MCP tool execution to UI (if available)"""
    if HAS_BROADCASTER and broadcaster:
        try:
            await broadcaster.broadcast_mcp_event(tool_name, arguments, result, status)
...
async def call_tool(name: str, arguments: Any) -> list[TextContent]:
    """Handle tool calls with UI broadcasting"""
    status = "success"
    result = {}
    ...
    # Broadcast to UI
    await broadcast_event(name, arguments, result, status)
